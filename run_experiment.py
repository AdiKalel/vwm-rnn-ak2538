...
def train(*, steps=TRAIN_STEPS, weights=None, loss_type=LOSS_TYPE,
          ...
          show_plot=True):
    """Train and return ``(weights, history)``; print and plot progress."""
    jax, jnp, optax = _jax()
    ...
    def objective(parameters, seed):
        key, theta, presence, inputs, initial = _trial_data(jax, jnp, set_size, seed)
        result = jax_run_trial(parameters, inputs, initial, DT_MS, SATURATION_RATE_HZ,
                               noise_type, noise_factor, key, True)
        return _loss_terms(jnp, result, theta, presence, loss_fn, loss_type)[0]

    objective = jax.jit(objective)
    history = []
    for step in range(steps):
        value, gradients = jax.value_and_grad(objective)(current, RANDOM_SEED + step)
        ...
        print(f"training step {step + 1}/{steps}: total_loss={value:.6g}", end="\r")
    print()
    if show_plot:
        _plot_training(history)
    return current, {"total_loss": history}
...
def _target_output(jnp, theta, presence):
    return jnp.stack((jnp.cos(theta), jnp.sin(theta)), axis=-1).reshape(-1) * jnp.repeat(presence, 2)
...
def _trial_report(weights=None, set_size=SET_SIZE, seed=RANDOM_SEED, noise_type=NOISE_TYPE,
    ...
    loss_fn = get_loss(loss_type, CUSTOM_LOSS if loss_type == "custom" else None)
    total, error, penalty, activation, decoded, mean_output = _loss_terms(jnp, result, theta, presence, loss_fn, loss_type)
